...
void CWebServer::Cmd_AddHardware(Json::Value &root)
{
	...
	else if ((htype == HTYPE_RFXLAN) || (htype == HTYPE_P1SmartMeterLAN) || (htype == HTYPE_YouLess) || (htype == HTYPE_RazberryZWave) || (htype == HTYPE_OpenThermGatewayTCP) || (htype == HTYPE_LimitlessLights) || (htype == HTYPE_SolarEdgeTCP) || (htype == HTYPE_WOL) || (htype == HTYPE_ECODEVICES) || (htype == HTYPE_WBHomaBridge)) {
		//Lan
		if (address=="")
			return;
	}
	...
	if (result.size()>0)
	{
		...
		for (itt=result.begin(); itt!=result.end(); ++itt)
		{
				...
				)
			{
				...
				if (switchtype==STYPE_Dimmer && (dType != pTypeLighting5 ||
                                                 (dSubType != sTypeWBRGB &&
                                                  dSubType != sTypeWBDimmer)))
				{
